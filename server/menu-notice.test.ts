...
describe('Menu Notice Banner', () => {
  beforeAll(async () => {
    const db = await getDb();
    if (!db) throw new Error('Database not available for testing');
  });

  it('should have menu_notice_enabled setting in database', async () => {
    const db = await getDb();
    if (!db) throw new Error('Database not available');

    const setting = await db.select()
      .from(siteSettings)
      .where(eq(siteSettings.settingKey, 'menu_notice_enabled'))
      .limit(1);

    expect(setting).toBeDefined();
    expect(setting.length).toBe(1);
    expect(setting[0].settingKey).toBe('menu_notice_enabled');
    expect(setting[0].settingType).toBe('boolean');
    expect(['true', 'false']).toContain(setting[0].settingValue);
  });

  it('should have menu_notice_text setting in database', async () => {
    const db = await getDb();
    if (!db) throw new Error('Database not available');

    const setting = await db.select()
      .from(siteSettings)
      .where(eq(siteSettings.settingKey, 'menu_notice_text'))
      .limit(1);

    expect(setting).toBeDefined();
    expect(setting.length).toBe(1);
    expect(setting[0].settingKey).toBe('menu_notice_text');
  ...
  it('should return correct structure from getMenuNotice query', async () => {
    const db = await getDb();
    if (!db) throw new Error('Database not available');
  ...
  it('should allow updating notice text', async () => {
    ...
    const testMessage = 'Test menu notice message';
    ...
    await db.update(siteSettings)
      .set({ settingValue: 'New seasonal menu items available! Try our special Jollof Rice with grilled fish.' })
      .where(eq(siteSettings.settingKey, 'menu_notice_text'));
  });
});
