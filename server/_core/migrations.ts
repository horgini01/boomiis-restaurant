...
async function seedEmailTemplates() {
  ...
  if (!db) {
    console.log('[Migration] Database not available, skipping email template seeding');
    return;
  }

  console.log('[Migration] Seeding email templates...');
  
  let inserted = 0;
  let skipped = 0;

  for (const [templateType, template] of Object.entries(DEFAULT_EMAIL_TEMPLATES)) {
    try {
      ...
      console.error(`[Migration] Error inserting template "${templateType}":`, error);
    }
  }
...
async function hasMigrationRun(migrationName: string): Promise<boolean> {
  ...
  try {
    const result = await db.select()
      .from(migrationsRun)
      .where(eq(migrationsRun.migrationName, migrationName))
      .limit(1);
...
async function markMigrationComplete(migrationName: string): Promise<void> {
  ...
  try {
    await db.insert(migrationsRun).values({
      migrationName,
      runAt: new Date(),
    });
...
export async function runDataMigrations() {
  console.log('[Migration] Checking for pending data migrations...');

  // Migration: Seed email templates
  const emailTemplatesMigration = 'seed_email_templates_v1';
  if (!(await hasMigrationRun(emailTemplatesMigration))) {
    console.log(`[Migration] Running: ${emailTemplatesMigration}`);
